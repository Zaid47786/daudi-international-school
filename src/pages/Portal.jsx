...
export default function Portal() {
  ...
  const role = user.role || "student";
  const content = role === "admin" ? <AdminPortalView section={sectionFromPath} payload={payload} refresh={load} go={setSection} /> : sectionFromPath === "dashboard" ? <DashboardView role={role} payload={payload} go={setSection} /> : sectionFromPath === "routine" ? <RoutineView payload={payload} /> : sectionFromPath === "attendance" ? <AttendanceView role={role} payload={payload} refresh={load} /> : sectionFromPath === "exams" ? <ExamsView payload={payload} /> : sectionFromPath === "results" || sectionFromPath === "gradebook" ? <ResultsView payload={payload} /> : sectionFromPath === "notices" ? <NoticesView payload={payload} /> : sectionFromPath === "calendar" ? <CalendarView payload={payload} /> : sectionFromPath === "fees" ? <FeesView payload={payload} /> : sectionFromPath === "digital-id" ? <DigitalIdView payload={payload} /> : sectionFromPath === "child" ? <StudentProfileView payload={payload} /> : sectionFromPath === "students" ? <StudentsView role={role} payload={payload} /> : <DashboardView role={role} payload={payload} go={setSection} />;
  return <PortalShell user={user} section={sectionFromPath} setSection={setSection} onLogout={logout}>{error && <div className="mb-5 rounded-xl bg-red-50 border border-red-100 px-4 py-3 text-sm text-red-700 flex items-center gap-2"><AlertCircle size={16} />{error}<button onClick={load} className="ml-auto"><RefreshCw size={15} /></button></div>}{content}</PortalShell>;
}
