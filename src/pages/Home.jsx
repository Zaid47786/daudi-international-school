        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-16 flex flex-col lg:flex-row items-center gap-12">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
              <Link to="/admissions" className="px-8 py-4 bg-gold text-navy font-bold rounded-full hover:bg-gold-dark transition-all duration-300 shadow-xl hover:shadow-gold/40 hover:scale-105 text-sm">
                Apply Now →
              </Link>
              <Link to="/about" className="px-8 py-4 border-2 border-white/30 text-white font-semibold rounded-full hover:border-gold hover:text-gold transition-all duration-300 text-sm">
                Learn More
              </Link>
            </div>
          </motion.div>
              ...
              {upcomingEvents.slice(0, 3).map((ev, i) => (
                <motion.div key={ev.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                  ...
                  <div className="flex-shrink-0 w-14 h-14 rounded-xl bg-royal-blue flex flex-col items-center justify-center text-white">
                    ...
                    <span className="text-xs font-bold text-center leading-tight px-1">{ev.date?.split(",")[0] || ev.date}</span>
                  </div>
                  <div>
                    <h3 className="font-bold text-navy text-sm mb-1">{ev.title}</h3>
                    <p className="text-gray-500 text-xs leading-relaxed">{ev.description}</p>
                  </div>
                </motion.div>
              ))}
            </div>
      ...
      <section className="py-16 bg-gradient-to-r from-royal-blue to-navy">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <p className="text-white/70 mb-8 text-lg">
            {settings.hero_badge} — Secure your child's future today.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link to="/admissions" className="px-10 py-4 bg-gold text-navy font-bold rounded-full hover:bg-gold-dark transition-all duration-300 shadow-xl hover:scale-105">
              Apply for Admission
            </Link>
            <Link to="/contact" className="px-10 py-4 border-2 border-white/40 text-white font-semibold rounded-full hover:border-gold hover:text-gold transition-all duration-300">
              Contact Us
            </Link>
          </div>
        </div>
