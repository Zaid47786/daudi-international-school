            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {upcoming.map((ev, i) => (
                <motion.div key={ev.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                  className={`rounded-2xl border-2 p-7 hover:shadow-xl transition-all duration-300 ${CARD_COLORS[ev.category] || "bg-gray-50 border-gray-200"}`}>
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="text-right">
                      <div className="flex items-center gap-1 text-gray-500 text-xs mb-1"><Calendar size={12} /> {ev.date}</div>
                      {ev.time && <div className="flex items-center gap-1 text-gray-500 text-xs"><Clock size={12} /> {ev.time}</div>}
                    </div>
                  </div>
                  <h3 className="font-bold text-navy text-xl mb-3">{ev.title}</h3>
                  {ev.description && <p className="text-gray-600 text-sm leading-relaxed mb-4">{ev.description}</p>}
                  {ev.location && (
                    <div className="flex items-center gap-2 text-xs text-gray-500">
                      <MapPin size={12} className="text-royal-blue" />{ev.location}
                    </div>
                  )}
                </motion.div>
              ))}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {past.map((ev, i) => (
                <motion.div key={ev.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
                  ...
                  {ev.description && <p className="text-gray-500 text-xs leading-relaxed">{ev.description}</p>}
                </motion.div>
              ))}
            </div>
          </div>
      ...
      {featured && (
        <section className="py-20 relative overflow-hidden"
          ...
          <div className="max-w-4xl mx-auto px-4 text-center">
            ...
            {featured.description && <p className="text-white/70 text-lg mb-8 leading-relaxed">{featured.description}</p>}
            <div className="inline-flex items-center gap-3 bg-gold/20 border border-gold/40 rounded-full px-6 py-3 text-gold font-semibold">
              <Calendar size={18} /> {featured.date} {featured.time && `· ${featured.time}`} {featured.location && `· ${featured.location}`}
            </div>
          </div>
        </section>
      )}
